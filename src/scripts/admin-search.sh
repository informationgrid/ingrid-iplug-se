...
# run it
exec "$JAVA" $JAVA_HEAP_MAX $INGRID_OPTS -Dlog4j.configuration=conf/log4j-search.properties -classpath "$CLASSPATH" $CLASS 8082 webapp --plugdescription conf/plugdescription-search.xml --descriptor conf/communication-search.xml
