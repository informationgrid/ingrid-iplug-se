...
public class TestNutchSearcher extends TestCase {
  ...
  protected void setUp() throws Exception {
    ...
    this.fConfiguration.set("plugin.folders",
        "../ingrid-nutch-plugins/src/plugin");
    this.fConfiguration
        .set(
            "plugin.includes",
  ...
  public void testGeoIntersect() throws Exception {
    IngridQuery query = QueryStringParser
        .parse("HVZ coord:intersect x1:1 x2:20 y1:10 y2:60");
    NutchSearcher searcher = new NutchSearcher(this.fIndex, "testId",
        this.fConfiguration);
    IngridHits hits = searcher.search(query, 0, 100);
    assertTrue(hits.length() == 0);
    ...
    query = QueryStringParser
        .parse("HVZ coord:intersect x1:9 x2:12 y1:50.1 y2:51");
    searcher = new NutchSearcher(this.fIndex, "testId", this.fConfiguration);
    hits = searcher.search(query, 0, 100);
    assertTrue(hits.length() > 0);

    query = QueryStringParser
        .parse("HVZ coord:intersect x1:11.2 x2:11.8 y1:50.1 y2:50.6");
    searcher = new NutchSearcher(this.fIndex, "testId", this.fConfiguration);
    hits = searcher.search(query, 0, 100);
    assertTrue(hits.length() == 0);
  ...
  public void testGeoInside() throws Exception {
    IngridQuery query = QueryStringParser
        .parse("HVZ coord:inside x1:1 x2:100 y1:1 y2:100");
    NutchSearcher searcher = new NutchSearcher(this.fIndex, "testId",
        this.fConfiguration);
    IngridHits hits = searcher.search(query, 0, 100);
    IngridHit[] hits2 = hits.getHits();
  ...
  public void testGeoInclude() throws Exception {
    IngridQuery query = QueryStringParser
        .parse("HVZ coord:include x1:1 x2:100 y1:1 y2:100");
    NutchSearcher searcher = new NutchSearcher(this.fIndex, "testId",
        this.fConfiguration);
    IngridHits hits = searcher.search(query, 0, 100);
    IngridHit[] hits2 = hits.getHits();
  ...
  public void testExplanation() throws Exception {
    ...
    IngridHit[] hits2 = hits.getHits();
    assertTrue(hits2.length > 0);
    IngridHitDetail detail = searcher.getDetail(hits2[0], query,
        new String[] { NutchSearcher.EXPLANATION });
  ...
  public void testGroupbyDataSource() throws Exception {
    IngridQuery query = QueryStringParser.parse("http " + "grouped:"
        + IngridQuery.GROUPED_BY_DATASOURCE);
    NutchSearcher searcher = new NutchSearcher(this.fIndex, "testId",
        this.fConfiguration);
    IngridHits hits = searcher.search(query, 0, 50);
    IngridHit[] hits2 = hits.getHits();
    for (int i = 0; i < hits2.length; i++) {
      IngridHitDetail detail = searcher.getDetail(hits2[i], query,
          new String[0]);
