...
import de.ingrid.nutch.crawl.bw.BWCrawlDbFilter;
import de.ingrid.nutch.crawl.bw.BWInjector;
import de.ingrid.nutch.crawl.bw.BWLinkDbFilter;
import de.ingrid.nutch.crawl.bw.BWUpdateDb;
import de.ingrid.nutch.crawl.bw.BWWebgraphFilter;
import de.ingrid.nutch.crawl.metadata.MetadataInjector;
import de.ingrid.nutch.crawl.metadata.ParseDataUpdater;
import de.ingrid.nutch.indexer.DeleteDuplicates;
import de.ingrid.nutch.indexer.IndexMerger;
import de.ingrid.nutch.mail.MailService;
import de.ingrid.nutch.segment.SegmentFilter;
import de.ingrid.nutch.tools.HostStatistic;
...
public class CrawlTool {
  ...
  public void crawl(Integer topn, Integer depth) throws IOException {
    ...
    IndexingJob indexer = new IndexingJob(_configuration);
    ...
    for (i = 0; i < depth; i++) { // generate new segment
      ...
      Path[] segmentPaths = generator.generate(crawlDb, segments, -1, topn, System
              .currentTimeMillis());
      if (segmentPaths == null || segmentPaths.length == 0) {
        LOG.info("Stopping at depth=" + i + " - no more URLs to fetch.");
        break;
      }
      Path segment = segmentPaths[0];
      if (LOG.isInfoEnabled()) {
    ...
    if (mergeSegments.length > 0) {
      ...
      indexer.index(crawlDb, linkDb, Arrays.asList(mergeSegments), true);
      if (indexes != null) {
        if (LOG.isInfoEnabled()) {
