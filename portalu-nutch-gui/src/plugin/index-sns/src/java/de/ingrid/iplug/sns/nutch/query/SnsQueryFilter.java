...
/**
 * @author mb
 * TODO rwe: The filter and plugin.xml must be adapted. Maybe, we need to enhance nutch api.
 */
public class SnsQueryFilter implements QueryFilter {
  ...
  private Map fTimeMap;

  private Map fGeoMap;

  private Set fBuzzwordSet;
  ...
  private boolean handleClause(BooleanQuery output, boolean search, Clause c) {
    ...
    if (indexField.equals(this.fX1)) {
      ...
      List list = (List) this.fGeoMap.get(indexField);
      if (list == null) {
        list = new LinkedList();
      }
      list.add(value);
      ...
      List list = (List) this.fTimeMap.get(indexField);
      if (list == null) {
        list = new LinkedList();
      }
      list.add(value);
      this.fTimeMap.put(indexField, list);
    } else if (this.fCommunityCode.equals(indexField)) {
      addClause(output, c, indexField, value);
   ...
   * @param set
   */
  private void addBuzzwordToBasicQuery(org.apache.lucene.search.Query query2, Set set) {
    for (Iterator iter = set.iterator(); iter.hasNext();) {
      Term term = (Term) iter.next();
      // query-basic
      if (term.field().equals("url") && (query2 instanceof BooleanQuery)) {
        BooleanClause[] clauses2 = ((BooleanQuery) query2).getClauses();
        for (int j = 0; j < clauses2.length; j++) {
          BooleanClause clause2 = clauses2[j];
          org.apache.lucene.search.Query query3 = clause2.getQuery();
          for (Iterator iter2 = this.fBuzzwordSet.iterator(); iter2.hasNext();) {
            String buzzwrd = (String) iter2.next();
            ((BooleanQuery) query3).add(new TermQuery(new Term("buzzword", buzzwrd)), Occur.SHOULD);
          }
        }
        break;
      }
   ...
   * @return
   */
  private Set extractTerms(org.apache.lucene.search.Query query2) {
    Set set = new HashSet();
    try {
  ...
  private void prepareGeo(BooleanQuery booleanQuery) {
    List list = (List) this.fGeoMap.get(this.fCoord);
    if (list != null) {
  ...
  private void prepareTime(BooleanQuery query) {
    List list = (List) this.fTimeMap.get(this.fTime);
    if (list == null) {
  ...
  private void prepareInsideTime(BooleanQuery query) {
    ...
    if (t1 != null && t2 != null) {
      ...
      org.apache.lucene.search.RangeQuery rangeQueryt0 = new org.apache.lucene.search.RangeQuery(termT0Min, termT0Max,
          true);
      org.apache.lucene.search.RangeQuery rangeQuery11 = new org.apache.lucene.search.RangeQuery(termT1, max, true);
      org.apache.lucene.search.RangeQuery rangeQuery12 = new org.apache.lucene.search.RangeQuery(min, termT2, true);
  ...
  private void prepareIncludeTimeQuery(BooleanQuery query) {
    ...
    if (t1 != null && t2 != null) {
      ...
      org.apache.lucene.search.RangeQuery rangeQuery11 = new org.apache.lucene.search.RangeQuery(min, termT1, true);
      org.apache.lucene.search.RangeQuery rangeQuery12 = new org.apache.lucene.search.RangeQuery(termT2, max, true);
  ...
  public void setConf(Configuration arg0) {
    ...
    this.fCommunityCode = properties.getProperty(IPlugSNSPlugin.COMMUNITY_CODE);
    ...
    this.fGeoMap = new HashMap();
    this.fTimeMap = new HashMap();
    this.fBuzzwordSet = new HashSet();
